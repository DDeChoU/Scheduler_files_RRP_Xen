...
* A full fledged AAF-Single Scheduler is now ready, few minor patches required !!
* Latest Update: The Updated Sched_AAF code now has xen_scheduler(s) like design but with timeslice-domain table in each PCPU
* **********************************/
...
struct AAF_dom
{
   ...
   /* spinlock_t dom_lock;*/
	...
	int k;
	spinlock_t Dom_lock; /* A domain lock for each partition */
};
...
static inline struct AAF_dom *get_AAF_dom(struct AAF_dom *dom_ptr)
{
	
	/* Iterate through the list of domains to see if it matches dom_id */
	
		return dom_ptr->dom->domain_id;
...
{
	printk("Switch testing in AAF_switch\n");
...
static s_time_t hyperperiod(struct AAF_dom *par)
{
	...
	* the list of partitions and calculate cumulative hyperperiod of the system */
	...
	temp.y=10000;
		...
		list_for_each_entry(dom,&AAF_PRIV_INFO(ops)->ndom,dom_list)
			{	
			prv->hp = hyperperiod(dom); /* cumulative hp of all the domains */
...
/********************************** INit PCPU ***********************
	...
	***** return ***
	* none */
static void AAF_init_pdata(const struct scheduler *ops,void *pdata,int cpu)
{
       printk("2nd Stage Testing in Init_pdata\n");
	unsigned long flags;
...
{
	printk("3rd stage testing in alloc_pdata\n");
	struct AAF_pcpu *pc;
...
{
	printk("5h stage testing in alloc_domdata\n");	
	struct AAF_dom *doms;
	...
	/*struct AAF_parition par[sizeof(struct AAF_parition)];*/
	...
	*/
	...
	/* AAF_single(ops); */ /* commented temporarily. */
...
{
	printk("Stage 1 Testing in Init\n");	
	struct AAF_private_info *prv;
...
{
	printk("6th stage testing in alloc_vdata\n");
...
static struct task_slice AAF_schedule(const struct scheduler *ops, s_time_t now, bool_t task_scheduled)
{
	...
	* in that table is merely the Domain ID as for now, we assume each domain has a single partition */
	printk("\n Error Check:AAF_SCHEDULE before smp_"); 
	unsigned int cpu = smp_processor_id();
	struct AAF_pcpu *pc = AAF_PCPU(cpu); /* we have access to the struct pcpu with the help of CPU number now */
	printk( "\nAlert in Do schedule\n");
	if(list_empty(&pc->time_list))
	{
		ret.time = -1;
		ret.task = IDLE(cpu);
		return ret; /*Returns null pointer when the list is done.*/
	}
	...
	/*BUG_ON(1);*/
	pars = get_AAF_dom(ts->dom_ptr); /* gets partition */
	/* A list head */
