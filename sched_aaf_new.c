...
struct AAF_partition
{
	...
	*/
	...
	int k;
	spinlock_t par_lock;
	double (*aaf_calc)(double alpha,int k); /*Moved to AAF_partition*/
};
...
static s_time_t hyperperiod(struct AAF_partition *par)
{
	...
	if(pc->hp==0)
	{
		pc->hp = par->avail.period;
	}
	else
	{
		/* Numerator of the LCM stored in temp_res */
		temp_res_num = (par->avail.period)*(pc->hp);
		temp_res_denom = (gcd(par->avail.period,pc->hp));
